...
class BtcDepositContent extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
  ...
  createAddress = () => {
    if (this.props.onCreateAddress) {
      this.props.onCreateAddress('BTC');
    }
  };
      ...
      <div className={classes.contentContainer}>
        <div className={classes.infoPaneContainer}>
          <InfoPane>
            <ul className={classes.infoPaneText}>
              <li>{I18n.translate('deposit_btc_address_warning')}</li>
              <li>{I18n.translate('deposit_btc_address_authenticity')}</li>
              <li>{I18n.translate('deposit_btc_address_count_warning')}</li>
              <li>
                {I18n.translate(
                  'deposit_min_label',
                  `${depositInfo.limits.deposit.min_deposit_amount} BTC`
                )}
              </li>
            </ul>
          </InfoPane>
        ...
        {this.props.depositInfo.address_list.length > 0 ? (
          <div className={classes.descriptionText}>
            {I18n.translate('deposit_btc_description_text')}
          </div>
        ) : null}
        ...
        >
          <div>
            {this.props.depositInfo.address_list.length > 0
              ? I18n.translate('deposit_create_another_address')
              : I18n.translate('deposit_create_address')}
          </div>
        </Button>
      </div>
    ) : (
...
export default withStyles(styles)(BtcDepositContent);
