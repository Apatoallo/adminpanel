...
class ExenDepositContent extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
  ...
  createAddress = () => {
    if (this.props.onCreateAddress) {
      this.props.onCreateAddress(this.props.token);
    }
  };
  ...
  render() {
    ...
    return depositInfo ? (
      <div className={classes.contentContainer}>
        <div className={classes.infoPaneContainer}>
          <InfoPane>
            <ul className={classes.infoPaneText}>
              <li>
                {I18n.translate(
                  'deposit_eth_address_warning',
                  this.props.token
                )}
              </li>
              <li>
                {I18n.translate(
                  'deposit_eth_address_authenticity',
                  this.props.token
                )}
              </li>
              <li>
                {I18n.translate(
                  'deposit_min_label',
                  `${depositInfo.limits.deposit.min_deposit_amount} ${
                    this.props.token
                  }`
                )}
              </li>
        ...
        {depositInfo.address_list && depositInfo.address_list.length > 0 ? (
          <div className={classes.descriptionText}>
            {I18n.translate('deposit_eth_description_text')}
          </div>
        ) : null}
        {depositInfo.address_list &&
          depositInfo.address_list.map((item, index) => (
            <ExpansionPanel
              key={item.address}
...
export default withStyles(styles)(ExenDepositContent);
