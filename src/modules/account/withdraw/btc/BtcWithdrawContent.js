...
class BtcWithdrawContent extends React.PureComponent {
  constructor(props) {
    super(props);
      ...
      if (!isDownshiftEvent(event.type, 'CLICK_ITEM')) {
        ...
        if (
          ...
          if (trimmedInput.length >= 26 && trimmedInput.length <= 35) {
            this.props.onValidateAddress('BTC', trimmedInput);
          }
        }
      }
    }
    ...
    return withdrawalInfo ? (
      <div className={classes.contentContainer}>
          ...
          <InfoPane>
            <ul className={classes.infoPaneText}>
              ...
              <li>
                {I18n.translate(
                  'withdraw_min_label',
                  `${withdrawalInfo.limits.withdraw.min_withdraw_amount} BTC`
                )}
              </li>
        ...
        <MuiDownshift
          getRootProps={() => ({})}
          getInputProps={() => ({
            label: I18n.translate('transfers_btc_address_label'),
            placeholder: I18n.translate('transfers_btc_address_placeholder'),
            helperText:
              this.addressSubmitted &&
        ...
        <NumberFormat
          ...
          )}: ${this.props.accountLine.available_balance} BTC)`}
          InputProps={{
            classes: {
              underline: classes.addressLabelUnderline,
              error: classes.error
            ...
            endAdornment: (
              <InputAdornment position="end">
                <div className={classes.priceAdornmentStyle}>BTC</div>
              </InputAdornment>
            )
          }}
          InputLabelProps={{
        ...
        <div className={classes.keyValueRow}>
          ...
          <div className={classes.value}>
            {`${withdrawalInfo.fees.withdraw_fee} BTC`}
          </div>
        </div>
        <div className={classes.keyValueRow}>
          <div className={classes.key}>{I18n.translate('withdraw_total')}</div>
          <div className={classes.value}>{`${this.getTotalAmount()} BTC`}</div>
        </div>
        <Button
          variant="contained"
          className={this.props.classes.withdrawButton}
...
export default withStyles(styles)(BtcWithdrawContent);
