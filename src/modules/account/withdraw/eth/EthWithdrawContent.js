...
class EthWithdrawContent extends React.PureComponent {
  constructor(props) {
    super(props);
      ...
      if (!isDownshiftEvent(event.type, 'CLICK_ITEM')) {
        ...
        if (
          ...
          if (trimmedInput.length >= 33 && trimmedInput.length <= 43) {
            this.props.onValidateAddress('ETH', trimmedInput);
          }
        }
      }
    }
    ...
    return withdrawalInfo ? (
      <div className={classes.contentContainer}>
          ...
          <InfoPane>
            <ul className={classes.infoPaneText}>
              ...
              <li>
                {I18n.translate(
                  'withdraw_min_label',
                  `${withdrawalInfo.limits.withdraw.min_withdraw_amount} ETH`
                )}
              </li>
        ...
        <MuiDownshift
          getRootProps={() => ({})}
          getInputProps={() => ({
            label: I18n.translate('transfers_eth_address_label', 'ETH'),
        ...
        <NumberFormat
          ...
          )}: ${this.props.accountLine.available_balance} ETH)`}
          InputProps={{
            classes: {
              underline: classes.addressLabelUnderline,
              error: classes.error
            ...
            endAdornment: (
              <InputAdornment position="end">
                <div className={classes.priceAdornmentStyle}>ETH</div>
              </InputAdornment>
            )
          }}
          InputLabelProps={{
        ...
        <div className={classes.keyValueRow}>
          ...
          <div className={classes.value}>
            {`${withdrawalInfo.fees.withdraw_fee} ETH`}
          </div>
        </div>
        <div className={classes.keyValueRow}>
          <div className={classes.key}>{I18n.translate('withdraw_total')}</div>
          <div className={classes.value}>{`${this.getTotalAmount()} ETH`}</div>
        </div>
        <Button
          variant="contained"
          className={this.props.classes.withdrawButton}
...
export default withStyles(styles)(EthWithdrawContent);
