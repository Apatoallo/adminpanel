...
class XlmWithdrawContent extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  validatorFields = [
    ...
    },
    {
      ...
      message: I18n.translate('validation_error_max_length', 28)
    },
    {
      field: 'destinationTag',
      method: () => {
  ...
  handleAddressInputChange = (input, event) => {
    if (!isDownshiftEvent(event.type, 'MOUSE_UP')) {
      ...
      if (
        ...
        if (trimmedInput.length >= 45 && trimmedInput.length <= 57) {
          this.props.onValidateAddress('XLM', trimmedInput);
        }
      }
    }
  };
  ...
  handleAddressSelectionChange = (item, event) => {
    ...
    if (trimmedInput.length >= 46 && trimmedInput.length <= 57) {
      this.props.onValidateAddress('XLM', trimmedInput);
    }
  };
  ...
  getTotalAmount = () => {
    ...
    const fee = parseFloat(this.props.withdrawalInfo.fees.withdraw_fee, 10);
    const tax = parseFloat(this.props.withdrawalInfo.fees.withdraw_tax, 10);
    return (amount + fee + tax).toFixed(7);
  };
  ...
  render() {
    ...
    return withdrawalInfo ? (
      <div className={classes.contentContainer}>
        ...
        >
          ...
          <DialogContent className={classes.dialogContent}>
              ...
              <InfoPane className={classes.confirmationInfoPane}>
                <div className={classes.confirmationNote}>
                  <p>
                    ...
                    {I18n.translate('withdraw_confirmation_dialog_xlm_warning')}
                  </p>
                </div>
              </InfoPane>
            )}
            <div className={classes.confirmationKeyValueArea}>
              ...
              <div className={classes.confirmationKeyValueRow}>
                <div className={classes.confirmationKey}>
                  {I18n.translate('withdraw_confirmation_dialog_memo_label')}
                </div>
                <div className={classes.confirmationValue}>
                  {this.state.destinationTag}
                </div>
        ...
        <div className={classes.infoPaneContainer}>
          <InfoPane>
            <ul className={classes.infoPaneText}>
              <li>
                <strong>{I18n.translate('withdraw_xlm_tag_warning')}</strong>
              </li>
              <li>
              ...
              <li>
                {I18n.translate(
                  'withdraw_min_label',
                  `${withdrawalInfo.limits.withdraw.min_withdraw_amount} XLM`
                )}
              </li>
        ...
        <MuiDownshift
          getInputProps={() => ({
            label: I18n.translate('transfers_xlm_address_label'),
            placeholder: I18n.translate('transfers_xlm_address_placeholder'),
            helperText:
                  ...
                  primary={
                    <div className={classes.listItemContainer}>
                      <div className={classes.addressAliasContainer}>
                        <div className={classes.aliasWrapper}>
        ...
        <TextField
          ...
          label={I18n.translate('transfers_xlm_tag_label')}
          InputProps={{
            classes: {
              underline: classes.addressLabelUnderline,
              error: classes.error
          ...
          InputLabelProps={{
            classes: {
              ...
              root: classNames(classes.addressLabel, {
                [classes.error]: validation.destinationTag.isInvalid
              })
            }
          }}
        />
        <Tooltip
          title={I18n.translate('withdraw_xlm_tag_warning')}
        ...
        >
          <div className={classes.checkboxLine}>
            ...
            >
              {I18n.translate('withdraw_override_no_memo')}
            </div>
          </div>
        </Tooltip>
        <TextField
        ...
        <NumberFormat
          ...
          )}: ${this.props.accountLine.available_balance} XLM)`}
          InputProps={{
            classes: {
              underline: classes.addressLabelUnderline,
              error: classes.error
            ...
            endAdornment: (
              <InputAdornment position="end">
                <div className={classes.priceAdornmentStyle}>XLM</div>
              </InputAdornment>
            )
          }}
          InputLabelProps={{
        ...
        <div className={classes.keyValueRow}>
          ...
          <div className={classes.value}>
            {`${withdrawalInfo.fees.withdraw_fee} XLM`}
          </div>
        </div>
        <div className={classes.keyValueRow}>
          <div className={classes.key}>{I18n.translate('withdraw_total')}</div>
          <div className={classes.value}>{`${this.getTotalAmount()} XLM`}</div>
        </div>
        <Button
          variant="contained"
          className={this.props.classes.withdrawButton}
...
export default withStyles(styles)(XlmWithdrawContent);
